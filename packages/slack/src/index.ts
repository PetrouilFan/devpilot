...
import { createOpencode, type ToolPart } from "@devpilot-ai/sdk"
...
console.log("🚀 Starting devpilot server...")
const devpilot = await createOpencode({
  port: 0,
})
console.log("✅ DevPilot server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await devpilot.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new devpilot session...")
    const { client, server } = devpilot
    ...
    console.log("✅ Created devpilot session:", createResult.data.id)
  ...
  console.log("📝 Sending to devpilot:", message.text)
