...
const managedConfigDir = process.env.DEVPILOT_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.DEVPILOT_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.DEVPILOT_CONSOLE_TOKEN
  else process.env.DEVPILOT_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})
...
it.effect("does not create global config when DEVPILOT_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:DEVPILOT_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.effect("does not try to install dependencies in read-only DEVPILOT_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("DEVPILOT_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
)

it.effect("installs dependencies in writable DEVPILOT_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("global config remains global when project config is disabled", () =>
  withConfigTree(
    ...
    withProcessEnv(
      "DEVPILOT_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const config = yield* Config.use.get()
...
// Note: preload.ts sets DEVPILOT_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
describe("DEVPILOT_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { DEVPILOT_CONFIG_DIR: undefined, DEVPILOT_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "DEVPILOT_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { DEVPILOT_DISABLE_PROJECT_CONFIG: "true", DEVPILOT_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed DEVPILOT_PERMISSION JSON used to crash
...
describe("DEVPILOT_PERMISSION env var", () => {
  it.instance("does not crash when DEVPILOT_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "DEVPILOT_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("DEVPILOT_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in DEVPILOT_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "DEVPILOT_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in DEVPILOT_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
