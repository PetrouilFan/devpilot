...
const cleanState = Effect.gen(function* () {
  ...
  delete process.env.DEVPILOT_CONFIG
  delete process.env.DEVPILOT_TUI_CONFIG
...
it.instance("project config takes precedence over DEVPILOT_TUI_CONFIG (matches DEVPILOT_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("DEVPILOT_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from DEVPILOT_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* FSUtil.Service
      const test = yield* TestInstance
