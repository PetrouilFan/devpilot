...
import { EventSequenceTable } from "@devpilot-ai/core/event/sql"
...
const originalEnv = {
  DEVPILOT_AUTH_CONTENT: process.env.DEVPILOT_AUTH_CONTENT,
  DEVPILOT_EXPERIMENTAL_WORKSPACES: process.env.DEVPILOT_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.DEVPILOT_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.DEVPILOT_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.DEVPILOT_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.DEVPILOT_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.DEVPILOT_EXPERIMENTAL_WORKSPACES).toBe("true")
