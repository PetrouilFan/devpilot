...
import type { TuiAttentionSoundPack } from "@devpilot-ai/plugin/tui"
...
const { allThemes, addTheme } = await import("@devpilot-ai/tui/context/theme")
...
`,
  ...
  process.env.DEVPILOT_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.DEVPILOT_PLUGIN_META_FILE!,
    )
