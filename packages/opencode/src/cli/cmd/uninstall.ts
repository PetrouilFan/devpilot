...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g devpilot-ai",
      pnpm: "pnpm uninstall -g devpilot-ai",
      bun: "bun remove -g devpilot-ai",
      yarn: "yarn global remove devpilot-ai",
      brew: "brew uninstall devpilot",
      choco: "choco uninstall devpilot",
      scoop: "scoop uninstall devpilot",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "devpilot", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# devpilot") || content.includes(".devpilot/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".devpilot/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".devpilot/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".devpilot"))
    ) {
      continue
    }
