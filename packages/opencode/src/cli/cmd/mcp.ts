...
import { ConfigMCPV1 } from "@devpilot-ai/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: devpilot mcp add")
      return
    }
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .devpilot/ subdirectory too)
  const candidates = [path.join(baseDir, "devpilot.json"), path.join(baseDir, "devpilot.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".devpilot", "devpilot.json"), path.join(baseDir, ".devpilot", "devpilot.jsonc"))
  }
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., devpilot x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "devpilot-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
