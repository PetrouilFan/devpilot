...
import { httpClient } from "@devpilot-ai/core/effect/layer-node-platform"
...
import { InstallationLocal, InstallationVersion } from "@devpilot-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@devpilot-ai/core/v1/config/console-state"
import { FSUtil } from "@devpilot-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@devpilot-ai/core/util/effect-flock"
...
import { Npm } from "@devpilot-ai/core/npm"
...
export class Service extends Context.Service<Service, Interface>()("@devpilot/Config") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.DEVPILOT_CONFIG && !Flag.DEVPILOT_CONFIG_DIR && !Flag.DEVPILOT_CONFIG_CONTENT) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "DEVPILOT_CONFIG_CONTENT") return "local"
        ...
        if (Flag.DEVPILOT_CONFIG) {
          yield* merge(Flag.DEVPILOT_CONFIG, yield* loadFile(Flag.DEVPILOT_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.DEVPILOT_CONFIG })
        }

        if (!Flag.DEVPILOT_DISABLE_PROJECT_CONFIG) {
        ...
        if (Flag.DEVPILOT_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from DEVPILOT_CONFIG_DIR", { path: Flag.DEVPILOT_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".opencode") || dir === Flag.DEVPILOT_CONFIG_DIR) {
        ...
        if (process.env.DEVPILOT_CONFIG_CONTENT) {
          const source = "DEVPILOT_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.DEVPILOT_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from DEVPILOT_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["DEVPILOT_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("DEVPILOT_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.DEVPILOT_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.DEVPILOT_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("DEVPILOT_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.DEVPILOT_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.DEVPILOT_DISABLE_PRUNE) {
