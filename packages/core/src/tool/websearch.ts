...
import { ToolFailure } from "@devpilot-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@devpilot/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.DEVPILOT_WEBSEARCH_PROVIDER === "exa" || process.env.DEVPILOT_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.DEVPILOT_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("DEVPILOT_EXPERIMENTAL") || truthy("DEVPILOT_ENABLE_EXA") || truthy("DEVPILOT_EXPERIMENTAL_EXA"),
    enableParallel: truthy("DEVPILOT_ENABLE_PARALLEL") || truthy("DEVPILOT_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `devpilot/${InstallationVersion}`,
