...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("devpilot-find")
  api.delete("devpilot-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("devpilot-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("devpilot-find", new Highlight(...rest))
    return true
  }
