...
export async function spawnWslSidecar(
  ...
  if (!opencode) throw new Error(`DevPilot is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export DEVPILOT_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export DEVPILOT_CLIENT=desktop",
    `export DEVPILOT_SERVER_USERNAME=${shellEscape(username)}`,
    `export DEVPILOT_SERVER_PASSWORD=${shellEscape(password)}`,
