...
import { createDevPilot } from "@devpilot-ai/sdk"
...
`)
  ...
  const devpilot = await createDevPilot({ port: 0 })
  ...
  try {
    ...
    const session = await devpilot.client.session.create()
    const result = await devpilot.client.session
      .prompt({
