...
```text
packages/opencode ---\
                      > @devpilot-ai/tui -> @devpilot-ai/sdk
packages/cli --------/
```
...
`packages/cli`, or `@devpilot-ai/core`.
...
- Do not replace private `packages/opencode` imports with `@devpilot-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@devpilot-ai/tui`.

## Ownership Boundary

### `@devpilot-ai/tui` Owns
...
Status: Completed. The private `@devpilot-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@devpilot-ai/tui`.
...
- The package has no dependency on `opencode`, `@devpilot-ai/cli`, or
  `@devpilot-ai/core`.
...
primitives now live in `@devpilot-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@devpilot-ai/tui/runtime`. Movable app, component,
...
`@devpilot-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@devpilot-ai/tui`. Configured references resolve through the new
...
`@devpilot-ai/tui`. The legacy host injects a narrow plugin host that retains
...
public `@devpilot-ai/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@devpilot-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@devpilot-ai/core`, or either
  executable package.
...
rg '@devpilot-ai/core|packages/opencode|packages/cli' packages/tui
